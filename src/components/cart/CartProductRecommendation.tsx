...
function CartProductRecommendation() {
    ...
    useEffect(() => {
      const fetchProducts = async () => {
        try {
          let getProducts = await ProductsService.getProducts();
                ...
                return (
                    <div key={p.id} className="row justify-content-around border-bottom pb-4 mb-4">
                        <Link href={"/product/" + p.id} className="col-2 p-1 mt-2">
                            <img src={p.image.url} width={60} height={60} className="img-fluid rounded-start p-0" alt={p.name} />
                        </Link>
                        <div className="col-5">
                            <div className="card-body">
                                <Link href={"/products/" + p.id} className="card-title mb-1 text-primary fw-bold text-decoration-none overflow-y-hidden" style={{ fontSize: "15px", height: "40px" }}>
                                        {p.name}
                                </Link>
                                <p className="card-text text-secondary overflow-hidden" style={{ height: "60px", fontSize: "14px" }}>{productDetails}</p>
                            </div>
                        </div>
                        ...
                        <div className="col-3 me-0 pe-0 row align-content-start justify-content-end">
                            ...
                            <span className="text-bg-danger p-2 mt-2 text-center rounded-2 col-12" style={{ fontSize: "12px" }}>50% offer</span>
