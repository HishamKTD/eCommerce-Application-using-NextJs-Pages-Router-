...
export class ProductsService extends ServiceBase {
  ...
  static getProducts = async () => {
    try {
      var prodResponse = await fetch(`${this.getUrl("/products")}`, {
        method: "GET",
      ...
      var products = await prodResponse.json();
  ...
  static getProductById = async (id: string) => {
    try {
      var prodResponse = await fetch(`${this.getUrl(`/products/${id}`)}`, {
        method: "GET",
        headers: this.headers,
      });
      ...
      var product = await prodResponse.json();
      return product;
    } catch (error: any) {
  ...
  static getCategories = async () => {
    try {
      ...
      var data = await categoryResponse.json();
      category = data.data;
      return category;
    } catch (error: any) {
